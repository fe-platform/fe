...
Implemented with **React 19** (`createRoot` / `root.render()`). The host calls `render()` and receives an unmount function; the React root is created and destroyed inside the MFE, so the host has no knowledge of the framework.
